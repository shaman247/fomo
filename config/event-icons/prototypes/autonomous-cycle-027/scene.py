...
for z in [-1.5,-.5,.5,1.5]:
 points=[[.6*math.cos(a),.6*math.sin(a),z] for a in np.linspace(0,math.pi,17)]
 for a,c in zip(points,points[1:]):rod('metal hoop',a,c,.028,'#7195A1',6)
...
for x in [-.27,.27]:
 for z in [-.85,.85]:
  ...
  # Solid broad leaves avoid edge-on flat leaves in orthogonal views.
  for sign in [-1,1]:
   ball('crop leaf',(x+sign*.10,.19,z),.12,'#70965D')
   part=S['parts'][-1]
   part['vertices']=[[xx,.19+(yy-.19)*.75,z+(zz-z)*.55] for xx,yy,zz in part['vertices']]
...
S['notes']=['Original illustrative low tunnel with four anchored hoops, ridge support, transparent plastic sheet and weighted long edges. Two open ends schematically show ventilation, not a winter-sealed configuration. Four representative plants remain within the cover. Dimensions and enlarged hoop thickness are pictogram choices, not event kit specifications or building guidance. Clear film is an illustrative silhouette with no optical refraction simulation.']
