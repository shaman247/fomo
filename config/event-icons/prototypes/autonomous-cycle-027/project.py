...
b=Path('.scratch/icon-cycles-20260909/cycle-027');out=b/'art'
...
def project(s):
 ...
 for p in s['parts']:
  ...
  for fidx in p['faces']:
   ...
   if p['name'] in ['metal hoop','buried hoop anchor','ridge support','plant stem','crop leaf','leaf vein','screen diamond','grille mark','tension rope','die edge','joining stitch','lace thread']:factor=1
 ...
 result=''.join(body)
 if glass:
  silhouette=unary_union(glass).simplify(.22,preserve_topology=True);d=''.join(ring(poly.exterior.coords) for poly in polygon_parts(silhouette));result=result+'<path d="'+d+'" fill="#A7C9CB" fill-opacity=".22" stroke="#A7C9CB" stroke-width="2.8" stroke-linejoin="round"/>'
