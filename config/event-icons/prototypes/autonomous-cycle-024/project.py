...
b=Path('.scratch/icon-cycles-20260909/cycle-024');out=b/'art'
...
def project(s):
 ...
 for p in s['parts']:
  ...
  for fidx in p['faces']:
   ...
   if p['name'] in ['potting mix','grit layer','soil']:factor=1.08 if norm[1]>.7 else .9
   if p['name'] in ['leaf vein','screen diamond','grille mark','tension rope','die edge','joining stitch','lace thread']:factor=1
   if any(k in p['name'] for k in ['piano key',' string',' sound hole',' peg']):factor=1
   col='#'+''.join(f'{int(v):02X}' for v in np.clip(base*factor,0,255));faces.append((poly,plane,col,p.get('paint_underlay',0)))
 tree=STRtree([f[0] for f in faces]);paints={}
 for i,(poly,plane,color,underlay) in enumerate(faces):
  visible=poly
  for j in tree.query(poly):
   if j==i:continue
   other,op,_,other_underlay=faces[j]
   # Only the piano keybed/white-key background may paint beneath opaque higher layers.
   # Their physical scene and full depth-clipped v1 remain the comparison reference.
   if underlay and n[1]>.3 and (other_underlay==0 or other_underlay>underlay):continue
   delta=op-plane
 ...
 for color,polys in paints.items():
  merged=unary_union([set_precision(p,.001) for g in polys for p in polygon_parts(g)])
  # Only subpixel white-key clearances are omitted in the icon projection.
  # The scene retains52separate white keys and their actual modeled clearances.
  if s['name']=='piano-trio' and color=='#EDE9DA':merged=merged.buffer(.055,join_style=2).buffer(-.055,join_style=2)
  geom=merged.simplify(.22,preserve_topology=True);parts=[geom] if geom.geom_type=='Polygon' else list(getattr(geom,'geoms',[]));d=''
  for poly in parts:
 ...
 result=''.join(body)
 if glass:
  silhouette=unary_union(glass).simplify(.22,preserve_topology=True);d=''.join(ring(poly.exterior.coords) for poly in polygon_parts(silhouette));result='<path d="'+d+'" fill="#A7C9CB" fill-opacity=".12"/>'+result+'<path d="'+d+'" fill="none" stroke="#A7C9CB" stroke-width="2.8" stroke-linejoin="round"/>'
