...
b=Path('.scratch/icon-cycles-20260909/cycle-081');out=b/'art'
...
def project(s):
 ...
 for color,polys in paints.items():
  geom=unary_union([set_precision(p,.00001) for g in polys for p in polygon_parts(g)]).simplify(.45 if s['name'] in ['seven-string-guitar'] else (.55 if s['name']=='balloon-twisting' else .20),preserve_topology=True);parts=[geom] if geom.geom_type=='Polygon' else list(getattr(geom,'geoms',[]));d=''
  for poly in parts:
