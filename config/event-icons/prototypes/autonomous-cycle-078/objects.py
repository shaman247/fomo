...
for c in [-1,0,1]:
 ...
 for i,(x,z) in enumerate(route):
  d=route[min(i+1,len(route)-1)]-route[max(i-1,0)];d/=np.linalg.norm(d);normal=np.array([-d[1],d[0]])
  pts.append(np.array([x,z])+normal*c*.00235)
 paths.append(np.array(pts))
# Solve shared strand heights against every actual offset-cord crossing. This
# avoids the overlapping lift envelopes of the initial study and gives the
# upper and lower bundles positive physical clearance, with ground support.
from scipy.optimize import linprog
N=len(route);constraints=[];bounds=[]
for ai,pa in enumerate(paths):
 for bj,pb in enumerate(paths):
  for i in range(N-1):
   for j in range(i+4,N-1):
    la=LineString(pa[i:i+2]);lb=LineString(pb[j:j+2]);hit=la.intersection(lb)
    if hit.geom_type!='Point':continue
    ci,cj=min(crossings,key=lambda q:abs(q[0]-i)+abs(q[1]-j));up=raised[ci]
    ta=la.project(hit,normalized=True);tb=lb.project(hit,normalized=True)
    row=np.zeros(N);row[i]+=1-ta;row[i+1]+=ta;row[j]-=1-tb;row[j+1]-=tb
    constraints.append(-row if up else row);bounds.append(-.00225)
for i in range(N-1):
 row=np.zeros(N);row[i]=1;row[i+1]=-1
 constraints.extend([row,-row]);bounds.extend([.0012,.0012])
solution=linprog(np.ones(N),A_ub=np.array(constraints),b_ub=np.array(bounds),bounds=[(.00115,None)]*N,method='highs')
assert solution.success,solution.message
height=solution.x
for c,path,col in zip([-1,0,1],paths,['#C69B61','#EFE0BD','#B9848C']):
 tube('continuous paper cord '+str(c),[(x,height[i],z) for i,(x,z) in enumerate(path)],.0011,col,n=8)
...
for j,(x,z) in enumerate([(.010,.008),(.020,.009),(.017,.016)]):
...
ellipsoid('clay tomato',(.020,.007,-.016),(.0045,.004,.0045),'#BF7564',n=16,rings=7)
...
scene('wooden-buoy-decoration',(3,6,8))
sc.S['camera_roll']=-35
...
p=sc.S['parts'][-1];p['vertices']=[[y,.022+z,-x] for x,y,z in p['vertices']];p['faces']=[list(reversed(f)) for f in p['faces']]
