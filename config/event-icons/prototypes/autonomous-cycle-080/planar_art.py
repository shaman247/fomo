...
art['coming-of-age-film']=frame('#CED8CE')+'<path d="M40 84h17V69h15V56h8" fill="none" stroke="#A77F69" stroke-width="6" stroke-linecap="round" stroke-linejoin="round"/><path d="m40 69 4 8 9 1-7 6 2 9-8-5-8 5 2-9-7-6 9-1Z" fill="#E6BB74"/><path d="m78 35 6 12 14 2-10 10 2 14-12-7-12 7 2-14-10-10 14-2Z" fill="#ECCB8F"/>'
...
(b/'sizes-final.json').write_text(json.dumps({p.name:dict(raw=len(p.read_bytes()),gzip_level_6=len(gzip.compress(p.read_bytes(),compresslevel=6,mtime=0))) for p in (b/'art').glob('*.svg')},indent=2)+'\n')
