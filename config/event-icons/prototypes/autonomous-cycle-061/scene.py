...
for c,scale in [((-.23,.21,.07),(.19,.105,.115)),((.17,.195,.09),(.15,.085,.105)),((-.07,.19,-.16),(.14,.08,.10))]:
 ellipsoid('harvested potato',c,scale,'#C99F73' if c[2]<0 else '#DAB98B' if c[0]<0 else '#C5A579');S['parts'][-1]['flat_paint']=True
 x,y,z=c
...
# Shorten only the upper wooden shaft/handle, preserving all soil and tine contacts.
for p in S['parts']:
 if p['name'] in ['wood shaft','open D handle']:
  p['vertices']=[[x,.43+(y-.43)*.67,z] for x,y,z in p['vertices']]
...
# Continuous cubic interpolation through the trunk centers eliminates angular disconnected-looking joints.
centers=np.array([a for a,r in trunk]);radii=np.array([r for a,r in trunk]);vs=[];n=14
for j in range(len(trunk)-1):
 p0=centers[max(0,j-1)];p1=centers[j];p2=centers[j+1];p3=centers[min(len(trunk)-1,j+2)]
 for t in np.linspace(0,1,7,endpoint=j==len(trunk)-2):
  c=.5*((2*p1)+(-p0+p2)*t+(2*p0-5*p1+4*p2-p3)*t*t+(-p0+3*p1-3*p2+p3)*t*t*t)
  rr=radii[j]*(1-t)+radii[j+1]*t
  vs.extend(c+rr*np.array([math.cos(a),0,math.sin(a)]) for a in np.linspace(0,2*math.pi,n,endpoint=False))
mesh('continuous curved trunk',vs,[[j*n+i,(j+1)*n+i,(j+1)*n+(i+1)%n,j*n+(i+1)%n] for j in range(len(vs)//n-1) for i in range(n)],wood)
...
# Quiet two-tone foliage avoids faceting at native sizes.
for p in S['parts']:
 if p['name']=='trimmed foliage pad':p['flat_paint']=True
