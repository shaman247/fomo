...
scene('oyster-shucking',(3,6,11))
box('folded cloth',(0,.018,0),(.72,.036,.57),'#91AAAD')
# Irregular cupped half shell has a continuous shell wall and edible interior.
n=40;angles=np.linspace(0,2*math.pi,n,endpoint=False);cx=-.12;cz=.02
outer=np.array([[cx+.24*math.cos(t)*(1+.06*math.sin(5*t)),.095,cz+.18*math.sin(t)*(1+.09*math.sin(3*t))] for t in angles])
bottom=np.array([[cx+.16*math.cos(t),.039,cz+.11*math.sin(t)] for t in angles]);inner=outer.copy();inner[:,0]=cx+(inner[:,0]-cx)*.88;inner[:,2]=cz+(inner[:,2]-cz)*.85;inner[:,1]=.086
...
panel('shell inner floor',inner,'#D7D4C1')
ellipsoid('oyster meat',(cx-.025,.102,cz),(.14,.033,.096),'#C9BD99',24,9);S['parts'][-1]['flat_paint']=True
# Short-bladed shucking knife placed beside shell on cloth, not a demonstrated grip.
rod('knife handle',(.25,.067,-.25),(.25,.067,-.035),.043,'#B59D83',24)
flat_extrude('knife blade',[(.222,-.035),(.278,-.035),(.278,.15),(.25,.20),(.222,.15)],.046,.057,'#BEC6C4')
box('knife guard',(.25,.062,-.033),(.115,.024,.022),'#A1AFB2')
...
ellipsoid('whole mozzarella',(-.11,.17,-.015),(.16,.125,.155),'#E5E2CF',28,11)
ellipsoid('second mozzarella',(.15,.14,.05),(.135,.10,.135),'#D5D8C7',28,11)
