...
b=Path('.scratch/icon-cycles-20260909/cycle-033');out=b/'art'
 ...
 for color,polys in paints.items():
  ...
  for poly in parts:
   if poly.geom_type=='Polygon' and poly.area>.045:
    # Caviar aggregate can paint beneath opaque beads: these internal cutouts
    # add bytes and antialias seams. Same scene/camera and visible outer edge.
    if s['name']=='caviar' and color=='#384140':poly=Polygon(poly.exterior)
    if s['name']=='caviar' and color=='#60665B':poly=poly.simplify(.16,preserve_topology=True)
