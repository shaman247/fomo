...
b=Path('.scratch/icon-cycles-20260909/cycle-007');out=b/'art'
...
def project(s):
 ...
 for p in s['parts']:
  ...
  for fidx in p['faces']:
   ...
   light=np.dot(norm,np.array([-.4,.8,.5])/np.linalg.norm([-.4,.8,.5]));factor=1 if any(k in p['name'] for k in ['petal','heart','bud','interior','wool','worm','seed','leaf','earth ball']) else 1 if p['name'] in ['warp','heddle','weft','tuner','pedal rod'] else 1.10 if light>.55 else 1 if light>0 else .82
   if p['name'] in ['tokkuri','choko']:factor=1.06 if norm[0]<.45 else .88
   if p['name'].startswith(('mic body','mic cap')):factor=1.05 if norm[0]<.45 else .90
   if p['name'] in ['shaker body','strainer shoulder','cap','fork handle','handle top']:factor=1.05 if norm[0]<.45 else .88
   if p['name'] in ['page block','verse ink','book cover']:factor=1
   if p['name'] in ['ball shell','pot','bowl']:factor=1.05 if norm[0]<.45 else .88
   if p['name'] in ['eyeshade face','elastic band']:factor=1
   if p['name'] in ['vegetables','ball interior']:factor=1
 ...
 for color,polys in paints.items():
  geom=unary_union(polys).simplify(.30 if s['name']=='inkle-loom' else .16,preserve_topology=True);parts=[geom] if geom.geom_type=='Polygon' else list(getattr(geom,'geoms',[]));d=''
  for poly in parts:
