...
b=Path('.scratch/icon-cycles-20260909/cycle-043');out=b/'art'
...
def project(s, bounds=None):
 right,up,n=basis(s['camera']);allv=np.concatenate([p['vertices'] for p in (bounds or s)['parts']]);xy=np.column_stack([allv@right,-allv@up]);lo=xy.min(0);hi=xy.max(0);scale=108/max(hi-lo);center=(lo+hi)/2;faces=[]
 glass=[]
 for p in s['parts']:
