...
b=Path('.scratch/icon-technique-ten-20260909');out=b/'art'
...
def project(s):
 ...
 for p in s['parts']:
  ...
  for fidx in p['faces']:
   f=vs[fidx];norm=np.cross(f[1]-f[0],f[2]-f[0]);length=np.linalg.norm(norm)
   ...
   light=np.dot(norm,np.array([-.4,.8,.5])/np.linalg.norm([-.4,.8,.5]));factor=1 if p['name'] in ['warp','heddle','weft','tuner','pedal rod'] else 1.10 if light>.55 else 1 if light>0 else .82
