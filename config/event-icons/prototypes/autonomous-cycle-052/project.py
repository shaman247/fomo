...
b=Path('.scratch/icon-cycles-20260909/cycle-052');out=b/'art'
...
def project(s):
 ...
 def fmt(v):return f'{round(v*2)/2:g}' if s['name']=='plant-swap' else f'{v:.1f}'.rstrip('0').rstrip('.')
 def ring(coords):
 ...
 for color,polys in paints.items():
  geom=unary_union([set_precision(p,.00001) for g in polys for p in polygon_parts(g)]).simplify(.5 if s['name']=='plant-swap' else .20,preserve_topology=True);parts=[geom] if geom.geom_type=='Polygon' else list(getattr(geom,'geoms',[]));d=''
  for poly in parts:
