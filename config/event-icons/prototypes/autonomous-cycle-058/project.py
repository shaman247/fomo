...
b=Path('.scratch/icon-cycles-20260909/cycle-058');out=b/'art'
...
def project(s):
 ...
 def fmt(v):return f'{round(v*2)/2:g}' if s['name']=='diorama' else f'{v:.1f}'.rstrip('0').rstrip('.')
 def ring(coords):
 ...
 for color,polys in paints.items():
  geom=unary_union([set_precision(p,.00001) for g in polys for p in polygon_parts(g)]).simplify(.5 if s['name']=='diorama' else .20,preserve_topology=True);parts=[geom] if geom.geom_type=='Polygon' else list(getattr(geom,'geoms',[]));d=''
  for poly in parts:
...
if __name__=='__main__':
 for name,s in SCENES.items():
  svg=project(s)
  if name=='boat-tour':
   k=svg.index('</title>')+len('</title>');body=svg[k:svg.rindex('</svg>')]
   card='<rect x="29" y="9" width="70" height="47" rx="4" fill="#DED8B9"/><path d="m33 41 18-7 12 10 14-16 18 9" fill="none" stroke="#B0BE9D" stroke-width="9"/><path d="M41 22q29 0 28 13t19 10" fill="none" stroke="#9B87A5" stroke-width="3" stroke-linecap="round" stroke-dasharray="3 5"/><circle cx="41" cy="22" r="4" fill="#9B87A5"/><circle cx="88" cy="45" r="4" fill="#9B87A5"/>'
   svg=svg[:k]+card+'<g transform="translate(0 25)">'+body+'</g><path d="M18 115q9 5 18 0t18 0 18 0 18 0 18 0" fill="none" stroke="#85B5BC" stroke-width="3" stroke-linecap="round"/></svg>\n'
