...
b=Path('.scratch/icon-cycles-20260909/cycle-008');out=b/'art'
...
def project(s):
 ...
 for p in s['parts']:
  ...
  for fidx in p['faces']:
   ...
   if p['name'] in ['cake','cake icing','piping bag','gathered bag']:factor=1.05 if norm[0]<.45 else .9
   if p['name'] in ['page block','journal line','heart entry','left collar','right collar','dress','trousers','price tag','frosting bead','spine band']:factor=1
