...
b=Path('.scratch/icon-cycles-20260909/cycle-082');out=b/'art'
...
def project(s):
 ...
 for p in s['parts']:
  ...
  for fidx in p['faces']:
   ...
   if s['name']=='balloon-twisting':factor=1.12 if light>.72 else 1
 ...
 def fmt(v):return f'{round(v*2)/2:g}' if s['name'] in ['seven-string-guitar'] else f'{v:.1f}'.rstrip('0').rstrip('.')
 def ring(coords):
 ...
 for color,polys in paints.items():
  geom=unary_union([set_precision(p,.00001) for g in polys for p in polygon_parts(g)]).simplify(.45 if s['name'] in ['seven-string-guitar'] else (.38 if s['name']=='hot-foil-stamping' else .20),preserve_topology=True);parts=[geom] if geom.geom_type=='Polygon' else list(getattr(geom,'geoms',[]));d=''
  for poly in parts:
