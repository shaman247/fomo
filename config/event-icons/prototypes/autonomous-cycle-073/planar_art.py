...
art['creature-feature']=frame('#D8DECA')+'<path d="M38 50Q28 48 28 35Q34 42 43 39Q64 31 85 39Q94 42 100 35Q100 48 90 50v28q-4 17-26 17T38 78Z" fill="#8D9B80"/><path d="m44 58 13 4-12 6Zm40 0-13 4 12 6Z" fill="#3E5A5C"/><path d="M46 76h36q-3 14-18 14T46 76Z" fill="#405B58"/><path d="m49 76 5 7 5-7Zm20 0 5 7 5-7Z" fill="#F3E2C3"/>'
# Lens and clue card are flat outline symbols,not a physically lit magnifier study.
art['crime-mystery-film']=frame('#DCCBBB')+'<rect x="33" y="39" width="39" height="49" rx="3" fill="#F2E3C9"/><path d="M41 48h23M41 57h15M41 66h19M41 75h11" fill="none" stroke="#A3917D" stroke-width="5" stroke-linecap="round"/><path d="m76 74 17 17" stroke="#876B62" stroke-width="10" stroke-linecap="round"/><circle cx="65" cy="62" r="20" fill="#C1D5D0" stroke="#496B73" stroke-width="7"/><path d="M59 56c0-9 13-9 13-1 0 5-7 4-7 10" fill="none" stroke="#55747C" stroke-width="4" stroke-linecap="round"/><circle cx="65" cy="73" r="2.5" fill="#55747C"/>'
...
(ROOT/'sizes-final.json').write_text(json.dumps({p.name:dict(raw=len(p.read_bytes()),gzip_level_6=len(gzip.compress(p.read_bytes(),compresslevel=6,mtime=0))) for p in (ROOT/'art').glob('*.svg')},indent=2)+'\n')
