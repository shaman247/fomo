...
b=Path('.scratch/icon-cycles-20260909/cycle-001');out=b/'art'
...
def project(s):
 ...
 for p in s['parts']:
  ...
  for fidx in p['faces']:
   ...
   light=np.dot(norm,np.array([-.4,.8,.5])/np.linalg.norm([-.4,.8,.5]));factor=1 if any(k in p['name'] for k in ['petal','heart','bud','interior']) else 1 if p['name'] in ['warp','heddle','weft','tuner','pedal rod'] else 1.10 if light>.55 else 1 if light>0 else .82
