...
b=Path('.scratch/icon-cycles-20260909/cycle-034');out=b/'art'
...
def project(s):
 ...
 for color,polys in paints.items():
  geom=unary_union([set_precision(p,.00001) for g in polys for p in polygon_parts(g)]).simplify(.20,preserve_topology=True);parts=[geom] if geom.geom_type=='Polygon' else list(getattr(geom,'geoms',[]));d=''
  for poly in parts:
   if poly.geom_type=='Polygon' and poly.area>.045:
    if s['name']=='beignets' and color in ['#D59A51','#E6A657','#B28144']:poly=Polygon(poly.exterior)
    if s['name']=='french-pastry' and color=='#DFA963':poly=Polygon(poly.exterior)
