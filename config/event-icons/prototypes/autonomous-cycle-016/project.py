...
b=Path('.scratch/icon-cycles-20260909/cycle-016');out=b/'art'
...
def project(s):
 ...
 for p in s['parts']:
  ...
  for fidx in p['faces']:
   ...
   if p['name'] in ['floating pigment','sample marbling','paw pad','paw toe','long stitch','cover label','spine label']:factor=1
 ...
 for color,polys in paints.items():
  geom=unary_union([p.buffer(.01) for g in polys for p in polygon_parts(g)]).buffer(-.01).simplify(.22,preserve_topology=True);parts=[geom] if geom.geom_type=='Polygon' else list(getattr(geom,'geoms',[]));d=''
  for poly in parts:
