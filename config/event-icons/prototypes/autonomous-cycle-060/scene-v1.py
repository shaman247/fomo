...
# Shallow paper food boat with six supported balls, not skewered dango.
box('food boat bottom',(0,.018,0),(.70,.035,.36),'#C6B58D')
for side in [-1,1]:
 panel('long folded boat side',[(-.36,.035,side*.18),(.36,.035,side*.18),(.42,.16,side*.255),(-.42,.16,side*.255)],'#DDD0A7')
 panel('boat end',[(side*.36,.035,-.18),(side*.36,.035,.18),(side*.42,.16,.255),(side*.42,.16,-.255)],'#B9A481')
r=.108
for x in [-.24,0,.24]:
 for z in [-.12,.12]:
  c=np.array([x,.145,z]);ball('round takoyaki batter',c,r,'#D3A168')
  # Sauced upper cap follows spherical surface; no detached discs.
  n=16;verts=[c+np.array([r*math.cos(t)*math.cos(a),r*math.sin(t)+.002,r*math.cos(t)*math.sin(a)]) for t in np.linspace(.45,math.pi/2,4) for a in np.linspace(0,2*math.pi,n,endpoint=False)]
  mesh('brown sauce',verts,[[j*n+i,(j+1)*n+i,(j+1)*n+(i+1)%n,j*n+(i+1)%n] for j in range(3) for i in range(n)],'#9D7258')
  # Two broad pale drizzle ribbons, geometric surface points keep toppings attached.
  for dz in [-.028,.031]:
   xs=np.linspace(-.075,.075,9);vs=[]
   for off in [-.008,.008]:
    for dx in xs:
     zz=dz+off+.009*math.sin(dx*32);yy=math.sqrt(max(0,r*r-dx*dx-zz*zz));vs.append(c+np.array([dx,yy+.004,zz]))
   mesh('pale drizzle',vs,[[i,i+1,9+i+1,9+i] for i in range(8)],'#EAD9B2');S['parts'][-1]['double_sided']=True;S['parts'][-1]['flat_paint']=True
