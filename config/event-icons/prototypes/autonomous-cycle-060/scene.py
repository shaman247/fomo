...
for x in [-.12,.12]:
 for z in [-.12,.12]:
  ...
  body_part=S['parts'][-1];verts=body_part['vertices'];capfaces=[f for f in body_part['faces'] if min(verts[i][1] for i in f)>c[1]+.02]
  mesh('brown sauce',verts,capfaces,'#9D7258');S['parts'][-1]['flat_paint']=True
