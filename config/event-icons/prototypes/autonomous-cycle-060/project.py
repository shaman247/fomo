...
b=Path('.scratch/icon-cycles-20260909/cycle-060');out=b/'art'
...
def project(s):
 ...
 def fmt(v):return f'{round(v*2)/2:g}' if s['name'] in ['diorama','takoyaki'] else f'{v:.1f}'.rstrip('0').rstrip('.')
 def ring(coords):
 ...
 for color,polys in paints.items():
  geom=unary_union([set_precision(p,.00001) for g in polys for p in polygon_parts(g)]).simplify(.45 if s['name'] in ['diorama','takoyaki'] else .20,preserve_topology=True);parts=[geom] if geom.geom_type=='Polygon' else list(getattr(geom,'geoms',[]));d=''
  for poly in parts:
...
if __name__=='__main__':
 for name,s in SCENES.items():
  svg=project(s)
  if name=='radio-theater':
   k=svg.index('</title>')+len('</title>');body=svg[k:svg.rindex('</svg>')];svg=svg[:k]+'<g transform="translate(3 16) scale(.94)">'+body+'</g><path d="M81 7h26q11 0 11 11v26q0 10-10 10H96L81 66l2-12h-3q-10 0-10-10V18q0-11 11-11" fill="#A49BB5"/><path d="M80 20q14 6 28 0v15q-1 14-14 18-13-4-14-18z" fill="#E4D6B8"/><path d="M85 29h4m10 0h4M87 38q7 8 14 0" fill="none" stroke="#A58C91" stroke-width="3" stroke-linecap="round"/></svg>\n'
