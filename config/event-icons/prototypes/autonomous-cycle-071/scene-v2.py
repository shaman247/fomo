...
scene('lantern-making',(4,5,7))
# Small craft-frame lantern: a real hollow cuboid with four joined edge frames,
# seated bottom panel and two-ended arch handle. Opaque color is a stylization
# of colored film,not an assertion of transparent-glass optics or a real flame.
w=.070;d=.065;h=.075;r=.0025
for x in [-w/2,w/2]:
 for z in [-d/2,d/2]:rod('vertical craft rib',(x,r,z),(x,h,z),r,'#A57F6B',n=10)
for y in [r,h]:
 for z in [-d/2,d/2]:rod('horizontal x rib',(-w/2,y,z),(w/2,y,z),r,'#A57F6B',n=10)
 for x in [-w/2,w/2]:rod('horizontal z rib',(x,y,-d/2),(x,y,d/2),r,'#A57F6B',n=10)
for j,z in enumerate([-d/2,d/2]):
 panel('colored film front/back',[[-w/2,r,z],[w/2,r,z],[w/2,h,z],[-w/2,h,z]],'#DCB487' if j else '#E1B4A1')
for j,x in enumerate([-w/2,w/2]):
 panel('colored film side',[[x,r,-d/2],[x,r,d/2],[x,h,d/2],[x,h,-d/2]],'#95B9A6' if j else '#A8C3AA')
box('bottom support',(0,.0035,0),(w,.002,d),'#B9987A')
# Curved pipe-cleaner handle attached to opposite upper ribs.
pts=[[w/2*math.cos(t),h+.030*math.sin(t),0] for t in [i*math.pi/20 for i in range(21)]]
tube('attached handle',pts,r,'#A57F6B',n=10)
# A little battery light inside; no candle. It is visible in top/rear review.
rod('mini light base',(0,.0045,0),(0,.010,0),.009,'#D9D1B8',n=16)
ball('mini light dome',(0,.012,0),.005,'#F5DBA1')
# A pale star cut-paper decoration is attached flush to the front film.
# It is a generic design choice,not a copy or an assertion of actual class design.
star=[]
for i in range(10):
 t=math.pi/2+i*math.pi/5;rr=.020 if i%2==0 else .009
 star.append([rr*math.cos(t),.041+rr*math.sin(t),d/2+.0001])
panel('paper star on film',star,'#F7E1AA')
# Loose folded colored craft sheet rests beside lantern to signal making.
flat_extrude('spare film sheet',[(.050,-.015),(.078,-.015),(.078,.022),(.050,.022)],0,.0005,'#E5BD8B')
S['notes']=['Metres; hollow70x65x75mm generic craft-frame lantern with twelve joined ribs,attached four film panels and seated base. Lowest ribs and spare sheet rest on ground. Handle attaches at both upper sides.','Colored film rendered as flat opaque color for pictogram clarity. Mini battery-light base rests inside; no candle,flame,glass technique or floating release. Original generic construction,not exact workshop design or material-kit guarantee.']
