...
b=Path('.scratch/icon-cycles-20260909/cycle-031');out=b/'art'
...
def project(s):
 if s["name"]=="mandolin" and tuple(s["camera"])==(1.6,.4,10):
  import copy
  plain=copy.deepcopy(s);plain["name"]="mandolin base";plain["parts"]=[p for p in plain["parts"] if p["name"]!="paired string"]
  raw=project(plain).replace("Mandolin Base","Mandolin")
  right,up,n=basis(s["camera"]);allv=np.concatenate([p["vertices"] for p in s["parts"]]);xy=np.column_stack([allv@right,-allv@up]);lo=xy.min(0);hi=xy.max(0);scale=108/max(hi-lo);center=(lo+hi)/2
  d=""
  for part in s["parts"]:
   if part["name"]!="paired string":continue
   v=np.array(part["vertices"]);nn=len(v)//2;ends=np.array([v[:nn].mean(0),v[nn:].mean(0)]);pts=(np.column_stack([ends@right,-ends@up])-center)*scale+64
   d+="M"+" ".join(f"{a:.1f}" for a in pts[0])+"L"+" ".join(f"{a:.1f}" for a in pts[1])
  return raw.replace("</svg>",f'<path d="{d}" fill="none" stroke="#E6D6AE" stroke-width=".26" stroke-linecap="round"/></svg>')
 ...
 for p in s['parts']:
  ...
  for fidx in p['faces']:
   ...
   if p['name'] in ['potting mix','grit layer']:factor=1.08 if norm[1]>.7 else .9
 ...
 for color,polys in paints.items():
  geom=unary_union([set_precision(p,.00001) for g in polys for p in polygon_parts(g)]).simplify(.03,preserve_topology=True);parts=[geom] if geom.geom_type=='Polygon' else list(getattr(geom,'geoms',[]));d=''
  for poly in parts:
