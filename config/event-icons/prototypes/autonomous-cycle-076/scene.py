...
box('open covers',(0,.0015,0),(.260,.003,.204),'#537F8C')
...
def inkline(name,xz,col,r=.00065):
 for a,b in zip(xz,xz[1:]):
  a,b=np.array(a),np.array(b);direction=b-a;direction=direction/np.linalg.norm(direction);normal=np.array([-direction[1],direction[0]])
  for t in range(6):
   aa=a+(b-a)*t/6;bb=a+(b-a)*(t+1)/6;outline=[aa+normal*r,bb+normal*r,bb-normal*r,aa-normal*r];vv=[(x,page_y(x)+.00002,z) for x,z in outline]
   mesh(name,vv,[[0,1,2],[0,2,3]],col);S['parts'][-1]['flat_paint']=True
...
inkline('abstract signed flourish',sig,'#526D80',.0020)
inkline('signature underline',[(.028,.043),(.061,.040),(.103,.040)],'#526D80',.0014)
# Uncapped writing pen rests on the shared ground beyond the book's near edge.
rod('pen barrel',(-.055,.0067,.116),(.066,.0067,.116),.0065,'#9C627C',n=12)
rod('metal pen nose',(-.075,.0067,.116),(-.055,.0067,.116),.00065,'#9AAAB0',n=12,r2=.0065)
rod('ink point',(-.079,.0067,.116),(-.075,.0067,.116),.00035,'#4F6976',n=8,r2=.00065)
rod('end plug',(.066,.0067,.116),(.071,.0067,.116),.0067,'#839DA3',n=12)
# Metal band is joined over barrel,not a detached cap.
rod('grip band',(-.056,.0067,.116),(-.051,.0067,.116),.0067,'#BDAD8D',n=12)
S['notes']=['Original representative lay-flat book260x204mm including covers. Broad pages curve toward the gutter; closed page blocks,cover and binding connect. Cover rests on ground.','Printed lines and original abstract signing flourish follow page height. No real signature,author name,cover image or readable text copied.','Uncapped pen lies beyond the near edge on the same ground plane. Grip band and end plug touch ground; narrow conical tip naturally has clearance. Grip band/end plug join the barrel. No hovering hand or writing-action claim.','Dimensions approximate generic stationery,not event-provided materials. LOC book structure excerpt inherited from75; original geometric approximation is not a conservation tutorial.']
