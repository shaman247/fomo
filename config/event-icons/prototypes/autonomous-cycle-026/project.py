...
b=Path('.scratch/icon-cycles-20260909/cycle-026');out=b/'art'
...
def project(s):
 right,up,n=basis(s['camera']);allv=np.concatenate([p['vertices'] for p in s['parts']]);xy=np.column_stack([allv@right,-allv@up]);lo=xy.min(0);hi=xy.max(0);scale=108/max(hi-lo);center=(lo+hi)/2;faces=[];underlays=[]
 glass=[]
 for p in s['parts']:
  ...
  for fidx in p['faces']:
   ...
   if p['name'] in ['leaf vein','screen diamond','grille mark','tension rope','die edge','joining stitch','lace thread','fabric','groove','eye','beak']:factor=1
   col='#'+''.join(f'{int(v):02X}' for v in np.clip(base*factor,0,255));faces.append((poly,plane,col));underlays.append(p.get('paint_underlay',False))
 tree=STRtree([f[0] for f in faces]);paints={}
 for i,(poly,plane,color) in enumerate(faces):
  visible=poly
  for j in tree.query(poly):
   if j==i:continue
   # Body fabrics can extend under their opaque attached face/motif pieces for the front icon camera.
   # Side/rear/top views retain complete depth clipping.
   if s['name']=='kimekomi' and n[2]>.7 and n[1]>.1 and underlays[i] and not underlays[j]:continue
