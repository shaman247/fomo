...
b=Path('.scratch/icon-cycles-20260909/cycle-009');out=b/'art'
...
def project(s):
 ...
 for p in s['parts']:
  ...
  for fidx in p['faces']:
   ...
   if p['name'] in ['film landscape','screen picture','score page','score staff','score note','moon emblem','clinic stripe','clinic door','door window','medical plus','mortar course','mortar joint','mural ground','mural landscape','mural river','fresh paint']:factor=1
   if p['name'] in ['tire','wheel hub']:factor=1
 ...
 for color,polys in paints.items():
  geom=unary_union([p.buffer(.01) for p in polys]).buffer(-.01).simplify(.22,preserve_topology=True);parts=[geom] if geom.geom_type=='Polygon' else list(getattr(geom,'geoms',[]));d=''
  for poly in parts:
