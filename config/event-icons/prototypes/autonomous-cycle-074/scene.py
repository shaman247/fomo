...
box('drawing paper',(0,-.0006,0),(.125,.0012,.125),'#E9DDC8')
for i,(x,z) in enumerate(legs):
 col=['#C88076','#769E94','#7C95AC'][i]
 rod('marker body'+str(i),(x,.009,z),(x,.082,z),.0058,col,n=12)
 rod('felt tip'+str(i),(x,0,z),(x,.009,z),.001,'#445C68',n=10,r2=.003)
 rod('marker cap'+str(i),(x,.076,z),(x,.087,z),.0062,col,n=12)
...
for i,((xa,za),(xb,zb)) in enumerate(zip(legs,legs[1:]+legs[:1])):
 ...
 box('marker binding'+str(i),(xa,.059,za),(.014,.007,.018),'#DED2AB')
...
for i,(cx,cz,col) in enumerate([(-.042,.026,'#C88076'),(.039,.025,'#769E94')]):
 ...
 tube('ink trace'+str(i),pts,.0009,col,n=4)
