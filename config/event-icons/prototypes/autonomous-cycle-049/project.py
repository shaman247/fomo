...
b=Path('.scratch/icon-cycles-20260909/cycle-049');out=b/'art'
...
def project(s):
 ...
 def fmt(v):return str(round(v)) if s['name']=='bodyweight-circuit' else f'{v:.1f}'.rstrip('0').rstrip('.')
 def ring(coords):
 ...
 for color,polys in paints.items():
  geom=unary_union([set_precision(p,.00001) for g in polys for p in polygon_parts(g)]).simplify(.5 if s['name']=='bodyweight-circuit' else .20,preserve_topology=True);parts=[geom] if geom.geom_type=='Polygon' else list(getattr(geom,'geoms',[]));d=''
  for poly in parts:
 ...
 result=''.join(body)
 if s['name']=='bodyweight-circuit':result+='<circle cx="101" cy="25" r="17" fill="#648FA1"/><circle cx="101" cy="25" r="12" fill="#F3E6CA"/><path d="M101 17v9l5 3M97 5h8" fill="none" stroke="#456874" stroke-width="3.5" stroke-linecap="round"/>'
 if glass:
