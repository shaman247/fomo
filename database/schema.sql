...
CREATE TABLE IF NOT EXISTS websites (
    ...
    force_chunked TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'Force chunked (two-pass) extraction regardless of the single-vs-chunked heuristic. For sites whose real event count sits just under LARGE_PAGE_THRESHOLD on content just under 2x MAX_CHUNK_CHARS, where a single call''s output budget cannot hold the page and the yield collapses non-deterministically. Per-site rationale: database/extraction_settings_rationale.md',
    max_records_per_chunk INT UNSIGNED DEFAULT NULL COMMENT 'Subdivide any extraction chunk holding more than N record headings (split only AT a heading). Opt-in: a chunk''s RESPONSE size scales with record COUNT, so a compact chunk of dense showtime cards overruns the output budget; a global cap was measured and rejected (+0.1% events for +83% chunks). Per-site rationale: database/extraction_settings_rationale.md',
    ...
    roving_organizer TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'The site is an organizer/promoter/lister whose one website_locations row is its office or home base, not where its events happen. Single-venue authority (processor Step 3.5) then refuses the loose Levenshtein match, so an unrecognised venue string stays unmapped instead of landing on the linked venue. Venue-less/virtual captions still fall back to the linked venue (Step 7). Set by hand after checking the crawl_events venue strings; never by heuristic.',
