...
- **Listing content hygiene (2026-09-19, `extractor.dedupe_repeated_paragraphs` / `prune_chunks`):** before chunking, later byte-identical copies of paragraphs ≥120 chars are dropped (a carousel duplicating the list, grid + list views; 2.5% of listing text, up to 33% on one site). After chunking, a chunk is not queued when it is *chrome-only* (no date token, no time token, no http link, no `EVENT DETAIL URL` marker) or *beyond-window* (≥3 fully-qualified dates, every date mention carries a year, and the earliest is past `FUTURE_WINDOW_DAYS` + 30 days). Validated against the 2026-09-18 run: 17 of 547 real packets pruned, all of which had returned zero events. A page whose every chunk is pruned resolves to an empty result like the "no events" shortcut.
- **Shared packet instructions:** static rules (detail-page rules, a site's chunk rules + notes) are hashed into the packet's `instructions`, written once per hash under `<work-dir>/instructions/`, and listed by `status` as `instructions_hash`; the prompt keeps only the per-packet text. `agent_extraction.py read --omit-schema --omit-instructions` prints just the header and source once a reviewer has read that hash. **Batch I/O:** `read-batch <manifest.json>` writes one document per batch (shared texts once, then every packet) and `submit-batch <manifest.json>` validates every response and lists each rejection; reviewers should use these rather than one `read`/`submit` round trip per packet, because the round trips, not the source text, dominated agent cost on 2026-09-18.
...
- **Known-complete skip (2026-09-19):** a crawl_event that already has occurrences and whose URL belongs to a live event carrying a real description AND a location is not a candidate — the merger keeps the existing description over the placeholder and never downgrades a resolved location, so the fetch could only re-derive what the merge inherits. This was 924 BPL + 547 Local Wine Events fetches per run (the same pages every crawl). Undated crawl_events are still fetched even when the URL is known.
- **Per-site cap:** at most `DETAIL_CRAWL_SITE_CAP` (env, default 80) detail fetches per website per run; the rest keep `detail_crawl_attempts = 0` and lead the next run. `0` disables (use for a targeted `--ids` run).
- `websites.skip_reenrichment = 1` — skips websites whose event URLs consistently fail or never carry prose (bot-protected ticketing sites, JS-rendered pages with no content, venue calendars whose detail pages are title/date/ticket-only). 21 sites were flagged 2026-09-19 on a 30-day measurement (≥90% of fetches returned no description while the listing already supplied dates and location); the criterion and list are in `.claude/decisions.md`.
