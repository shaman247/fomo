...
- **Extractor's geographic rule was a silent filter** (fixed 2026-05-16, but watch for regression). If a Hudson Valley / LI / N.NJ / Fairfield CT venue suddenly drops to 1-2 events extracted from a healthy-sized page, check `pipeline/extractor.py` `get_prompt()` for "NYC area" instead of the full metro list.
